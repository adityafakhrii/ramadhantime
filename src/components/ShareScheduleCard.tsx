...
export function ShareScheduleCard({ times, city }: ShareScheduleCardProps) {
  ...
  return (
    <div className="space-y-3">
      {/* Shareable card (rendered off-screen style but visible for preview) */}
      <div
        ref={cardRef}
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {PRAYER_LABELS.map((p) => (
            ...
            >
              <span style={{
                color: p.key === 'Maghrib' ? '#E0AA3E' : '#CCDDEE',
                fontSize: '14px',
                fontWeight: 600,
              }}>
                {p.label}
              ...
              <span style={{
                color: p.key === 'Maghrib' ? '#E0AA3E' : '#FFFFFF',
                fontSize: '16px',
                fontWeight: 700,
      ...
      <button
        onClick={handleShare}
        className="w-full py-3 rounded-xl bg-foreground text-background font-semibold text-sm hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
      >
