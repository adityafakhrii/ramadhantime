...
export const DailyQuote = ({ isRamadhan = false }: DailyQuoteProps) => {
    const dailyQuote = useMemo(() => {
        const pool = isRamadhan ? [...GENERAL_QUOTES, ...RAMADHAN_QUOTES] : GENERAL_QUOTES;
        const today = new Date();
        const dayIndex = (today.getDate() - 1) % pool.length;
